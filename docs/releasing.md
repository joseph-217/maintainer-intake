...
For tokenless CI publication, configure npm trusted publishing for GitHub repository `joseph-217/maintainer-intake` and workflow filename `publish.yml`, with `npm publish` allowed. Then dispatch **Publish npm** with the exact annotated version tag. The workflow uses a GitHub-hosted runner, grants only `contents: read` and `id-token: write`, checks tag/package/major-tag identity, runs the full verification lane, and publishes using npm's short-lived OIDC credential.
